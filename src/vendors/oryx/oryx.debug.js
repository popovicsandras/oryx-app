...
ORYX.Core.SVG.Label = Clazz.extend({
			...
			if (this.isVisible) {
				...
				if (this.node.ownerDocument) {
					...
					if (this.fitToElemId) {
						...
						var maxWidth = parseInt(this.node.getAttributeNS('http://activiti.org/bpmn', 'max-width'));
						var maxHeight = parseInt(this.node.getAttributeNS('http://activiti.org/bpmn', 'max-height'));
	...
	_checkFittingToReferencedElem: function(n) {
		try {
			...
			//if (/Firefox[\/\s](\d+\.\d+)/.test(navigator.userAgent) && new Number(RegExp.$1)>=3) {
                ...
                var refNode = n || this.node.ownerDocument.getElementById(this.fitToElemId);
				...
				if (refNode) {
                    ...
                    // Calculate max lines fitting into ref node,
                    // reserve 1 line for top/bottom spacing
                    var maxLines = Math.floor(refbb.height / fontSize) - 1;
                    var lines = 0;
                    while (newtspans.length > 0) {
                        ...
                        if (lines > maxLines) {
                            ...
                            if (lastChild.textContent.length > 3) {
                                lastChild.textContent = lastChild.textContent.substring(0, lastChild.textContent.length - 4) + ' ...';
                            } else {
...
ORYX.Core.StencilSet.StencilSet = Clazz.extend({
    ...
    construct: function(source, modelMetaData, editorId){
		...
		if(ORYX.CONFIG.BACKEND_SWITCH) {
			var stencilType;
			if (this._source.indexOf("xforms") !== -1) {
				stencilType = "xforms";
				this._baseUrl = "editor/stencilsets/xforms/";
				this._source = "stencilsets/xforms/xforms.json";
			} else if (modelMetaData.model.modelType == 'form-flow') {
				stencilType = "form-flow";
			...
			new Ajax.Request(ACTIVITI.CONFIG.contextRoot + "/app/rest/stencil-sets/" + modelMetaData.stencilSetId + "/editor?version=" + Date.now(), {
	            asynchronous: false,
	            method: 'get',
    ...
    findRootStencilName: function(){
        ...
        var rootStencil = Object.values(this._stencils).find(function(stencil){
...
/** @lends ORYX.Core.AbstractShape.prototype */
{
	...
	construct: function(options, stencil, facade) {
		...
		var stencilIdsToValidate = ['UserTask', 'ServiceTask', 'ScriptTask'];
		if (stencilIdsToValidate.indexOf(this._stencil.idWithoutNs()) >= 0) {
		    var isForCompensationKey = 'oryx-isforcompensation';
		    if (!this.properties[isForCompensationKey]) {
		        this.propertiesChanged['activiti-bpmn-disable-compensation'] = true;
		    }
		    
		    var isMultiInstanceKey = 'oryx-multiinstance_type';
		    if (!this.properties[isMultiInstanceKey]) {
                this.propertiesChanged['activiti-bpmn-disable-multiinstance'] = true;
            }
		}
...
ORYX.Editor = {
	...
	_createCanvas: function(stencilType, canvasConfig) {
        if (stencilType) {
            ...
            stencilType = Object.values(this.getStencilSets())[1].findRootStencilName();
        }
		...
		if(this.node.ownerDocument) {
			...
			this.propertiesChanged.each((function(propChanged) {
			    if (propChanged.value) {
			        if ('activiti-bpmn-disable-compensation' === propChanged.key) {
			            
			            //get the SVG element
                        var svgElem = this.node.ownerDocument.getElementById(this.id  + 'compensation');
                        
                        //if the SVG element can be found
                        if (svgElem && svgElem.ownerSVGElement) { 
                            svgElem.setAttributeNS(null, 'display', 'none');
                        }
                        
			        } else if ('activiti-bpmn-disable-multiinstance' === propChanged.key) {
        ...
        onSelectionChanged: function(event) {
                ...
                if(selection.any(function(s){ return s instanceof ORYX.Core.Node && s.getStencil().id().endsWith("Lane")})){
                        ...
                        lanes.each(function(lane){
                                var parentPool = this.getParentPool(lane);
                                if (parentPool) {
                                        pools.push(parentPool);
                                }
                        }.bind(this));
